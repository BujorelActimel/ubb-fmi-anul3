...
transformations, the WebAssembly execution model, and the snapshot-based approach
used by the reversible debugger.
...
diagnostics.  Pseudo++'s linter traverses the tree looking for these sentinel nodes
and reports their location to the user.
...
prerequisite for the debugger's snapshot mechanism (Section~\ref{sec:snapshots}).
...
\begin{tikzpicture}[
  ...
  \node[box] (src)   {Source};
  \node[box, below=of src]  (lint)  {Linter};
...
selection (\texttt{if}), and a single repetition construct.  A practical consequence
is that the four loop forms commonly found in procedural languages ---
\textit{for}, \textit{while}, \textit{do-while}, and \textit{repeat-until} --- are
mutually interconvertible: any loop can be rewritten using any other form, at the
...
WebAssembly (Wasm) \cite{wasm-spec} is a binary instruction format for a
stack-based virtual machine, standardised by the W3C in 2019.  Its design goals are
...
which branch is taken, how a loop counter evolves.
...
\subsection{Per-Step State Capture}

At each Step Into, the runtime first serialises the complete program state into
a \texttt{runtime\_snapshot\_t} structure before advancing execution.  A snapshot
records:

\begin{itemize}
  \item the variable environment: name, value, and type for every variable currently
        in scope;
  \item the execution stack: frame type, phase counter, loop bounds, loop counter,
        loop variable name, and the identity of the current CST node for each
        active frame;
  \item auxiliary I/O state: the multi-variable read index and a pending-input flag;
  \item the current line number and execution-state enum.
\end{itemize}

The snapshot identifier is stored by the JavaScript \texttt{PseudoDebugger} class
alongside the cached variable values and condition-evaluation metadata for that step.
Variable values are cached on the JavaScript side so that the variable panel can be
refreshed without an additional round-trip to the Wasm module.
