...
\node[cmod, fill=teal!5] (rt) at (2.0,  3.9) {runtime + debugger};
\node[cmod, fill=teal!5] (tr) at (6.5,  3.9) {transpiler};
\node[cmod, fill=teal!5] (eq) at (11.0, 3.9) {equivalence};

%% Foundation row (y = 2.3)
\node[cmod] (parse) at (2.0,  2.3) {parser (Tree-sitter)};
\node[cmod] (lint)  at (6.5,  2.3) {linter};
\node[cmod] (io)    at (11.0, 2.3) {I/O abstraction};

%% Utilities row (y = 0.7)
\node[cmod] (env)  at (2.0,  0.7) {environment};
\node[cmod] (str)  at (6.5,  0.7) {string};
\node[cmod] (hmap) at (11.0, 0.7) {hashmap};
...
\node[product, fill=orange!9, draw=orange!50,
  ...
  (cli) at (10.5, 9.4)
  {{\bfseries CLI}\\[4pt]
   {\footnotesize run $\cdot$ lint $\cdot$ parse}\\[1pt]
   {\footnotesize transpile $\cdot$ equivalence}};
...
\begin{description}
  \item[\texttt{linter}] Normalises the raw source text before parsing: it
  ...
  \item[\texttt{debugger}] Extends the interpreter with snapshot-based
        reversible execution, variable inspection, and JSON serialisation of
        runtime state for the browser front end.
  ...
  \item[\texttt{cli}] The command-line entry point, supporting \texttt{run},
        \texttt{transpile}, \texttt{debug}, \texttt{parse-tree}, and
        \texttt{equivalence} sub-commands.
\end{description}
...
the complete syntax of the Romanian high-school pseudocode dialect, following
the conventions used in the Romanian baccalaureate exam \cite{romanian-curriculum}.
...
The official Romanian high-school pseudocode notation \cite{romanian-curriculum},
as it appears in textbooks and on baccalaureate exam papers, uses a rich set
...
The linter is the bridge between the two representations.  It accepts source in
...
The linter maintains a static hash map that maps Unicode byte sequences to their
...
\hline
\end{tabular}
\caption{Selected substitution rules applied by the linter in Pass 1.
...
The public API exposes three execution modes:
...
state inspection, following the snapshot-based approach to reversible debugging
described by Lewis \cite{lewis2003omniscient} and surveyed by Engblom
\cite{reversible-debugging}.  The web editor exposes three operations:
\textbf{Step Into} (advance one visible action), \textbf{Continue} (run to
completion or the next input request), and \textbf{Stop}.  After each step,
the front end displays the currently executing line, the full variable table,
which variables changed since the last step, and --- when the step involved
evaluating a condition --- the condition text and its boolean result.

This is made possible by a \emph{snapshot-based} architecture in the C runtime
that captures the complete interpreter state before every step.

\paragraph{Snapshot structure.}
A \texttt{runtime\_snapshot\_t} captures the complete interpreter state at a
given execution point:

\begin{itemize}
  \item All variable bindings (name, value, and type) as a
        \texttt{var\_info\_t} array.
  \item The full execution stack: for each frame, its type, phase,
        \texttt{child\_idx}, loop counters, and the \emph{start byte} of the
        corresponding CST node.
  \item The auxiliary fields \texttt{read\_var\_index},
        \texttt{has\_pending\_read}, \texttt{current\_line}, and
        \texttt{exec\_state}.
\end{itemize}

CST nodes are identified by their \texttt{start\_byte} offset (a
\texttt{uint32\_t}) rather than by pointer.  On restoration,
\texttt{find\_node\_by\_start()} locates the matching node in the unchanged
CST.  Because the CST is never modified after parsing, this identity scheme is
safe and avoids serialising the full tree.

\paragraph{Circular buffer.}
Snapshots are stored in a fixed-size circular buffer of \texttt{MAX\_SNAPSHOTS}
entries, giving the JavaScript layer a history of past states it can replay
without re-executing the program from the beginning.  When the buffer is full,
...
\begin{enumerate}
  \item Runs the source through the linter and parser to obtain a CST.
...
\begin{itemize}
  ...
  \item \textbf{Step / Step Over / Step Back} --- calls the corresponding
        WASM step function, then queries the runtime for the current line,
        variable table (JSON), and condition information to update the
        highlighted line and the variable inspector panel.
...
The Visual Studio Code extension provides the same capabilities as the web
editor but integrated into the developer's local environment.  It bundles
the native CLI binary (compiled for the target platform) and invokes it as a
child process.  The extension contributes:

\begin{itemize}
  \item Syntax highlighting, through a TextMate grammar derived from the
        Monarch tokeniser rules.
  \item A \textbf{Run Pseudo++} command that executes the active file with
        the bundled CLI and streams output to a dedicated terminal.
  \item A \textbf{Transpile} command that runs the CLI's \texttt{transpile}
        sub-command and opens the result in a new editor tab.
\end{itemize}
