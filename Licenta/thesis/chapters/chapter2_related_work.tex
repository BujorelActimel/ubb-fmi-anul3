...
classes are browser-based compilers for the two target exam languages,
...
about arithmetic, nested loops, and sorting algorithms expressed as
...
Transpilation from an educational notation to a standard programming
language has been explored primarily in the block-based domain.  Tools
such as Blockly and Snap! generate Python or JavaScript from block
programs; MIT App Inventor similarly produces Java-compatible bytecode
from a block graph.  In all of these cases the input is visual and
structured-by-construction, so type inference and syntactic normalisation
are either trivial or unnecessary.

The text-based pseudocode domain is less explored.  Ben-Ari
\cite{benari2001} identifies the conceptual gap between pseudocode and
executable code as a persistent difficulty for learners and advocates for
tools that make the connection explicit and mechanical, but the emphasis
in the literature has been on pedagogy rather than on building such
tools.  Flowgorithm's export capability (Section~\ref{sec:pseudocode-tools})
is the only directly comparable implementation, but it starts from a
graphical flowchart rather than from text.

Pseudo++'s transpiler addresses this gap for the specific case of the
Romanian baccalaureate dialect.  The two-pass design --- a type-collection
pass followed by a code-emission pass --- handles the mismatch between
the dynamically typed pseudocode and the statically typed target languages
(C, C++, Pascal) without requiring explicit type annotations from the
student.  The loop-equivalence module additionally provides an interactive
implementation of a transformation that the curriculum requires students
to perform on paper, and for which no automated counterpart exists in any
...
Scratch           & \checkmark & $\times$   & $\times$   & $\times$   & \checkmark & \checkmark \\
Blockly           & $\times$   & $\times$   & \checkmark & $\times$   & \checkmark & \checkmark \\
...
combination of execution, step-through debugging, transpilation, and
static analysis in a single browser-based, open-source tool.  Flowgorithm
comes closest --- execution, debugger, and transpiler --- but it operates
on graphical flowcharts rather than text pseudocode and ships only as a
desktop application.  No other text-based tool combines all four
processing modes.
...
any algorithm can be tested.  None of the surveyed tools implement
loop-equivalence transformations as an interactive feature, despite
that being an explicit examination topic in the Romanian curriculum.
