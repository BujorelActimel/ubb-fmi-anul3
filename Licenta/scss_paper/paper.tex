...
\def\lstBoxV{\textSFxi}   % │ U+2502
\def\lstBoxTL{\textSFi}   % ┌ U+250C
\def\lstBoxBL{\textSFii}  % └ U+2514
\def\kw#1{{\bfseries #1}}


% Vertical spacing between floats and body text
...
\title{Pseudo++: An Open-Source Toolchain for Romanian Baccalaureate Pseudocode}
...
command-line interface and the browser-based web editor.  It comprises nine
...
constructs; a static analyzer reporting twelve diagnostic categories without
executing the program; a browser-based editor built on Monaco, running entirely offline; and a CLI
for scripting.
...
transpilation, loop-equivalence transformation, and static analysis for this
dialect in a single browser-based, installation-free environment.
\end{abstract}
...
papers.  The dialect uses Romanian keywords
...
box-drawing characters (\texttt{│}, $\blacksquare$) for
...
Section~\ref{sec:components} details the nine core components.
...
program reads a natural number \texttt{n} then, for each of its digits,
reads an additional value and extracts a specific digit from it using a
nested for loop and floor division, accumulating the result in \texttt{m}.

\begin{lstlisting}[language=pseudo, caption={A 2020 baccalaureate training problem in canonical Unicode notation.}, label=lst:example]
(*\kw{cite\c{s}te}*) n
p(*$\leftarrow$*)1; m(*$\leftarrow$*)0; k(*$\leftarrow$*)0
(*\lstBoxTL\kw{c\^{a}t}*) timp n(*$\neq$*)0 (*\kw{execut\u{a}}*)
(*\lstBoxV*) (*\kw{cite\c{s}te}*) x
(*\lstBoxV\lstBoxTL*)pentru i(*$\leftarrow$*)1,k (*\kw{execut\u{a}}*)
(*\lstBoxV\lstBoxV*) x(*$\leftarrow$*)[x/10]
(*\lstBoxV\lstBoxBL$\blacksquare$*)
(*\lstBoxV\lstBoxTL\kw{dac\u{a}}*) x(*$\neq$*)0 atunci c(*$\leftarrow$*)x%10
(*\lstBoxV\lstBoxV*)altfel c(*$\leftarrow$*)n%10
(*\lstBoxV\lstBoxBL$\blacksquare$*)
(*\lstBoxV*) m(*$\leftarrow$*)c*p+m
(*\lstBoxV*) n(*$\leftarrow$*)[n/10]
(*\lstBoxV*) p(*$\leftarrow$*)p*10; k(*$\leftarrow$*)k+1
(*\lstBoxBL$\blacksquare$*)
scrie m
\end{lstlisting}
...
supported.  The dialect has no user-defined functions, arrays, or types.
...
box-drawing characters (│ as indentation, $\blacksquare$ as a block
...
Ben-Ari~\cite{benari2001} advocates for executable, feedback-rich environments
as a means of supporting constructivist learning in CS education.  Pseudo++
operationalises this principle for the specific case of the Romanian
baccalaureate dialect, combining execution, debugging, and static feedback in a
single tool.
...
(Fig.~\ref{fig:arch}).  The \emph{utilities layer} (\texttt{value},
\texttt{environment}, \texttt{diagnostic}) provides the core runtime value
type, variable store, and shared diagnostic record used by all upper layers.  The
...
\node[box] (val)  at (1.4, 0.3) {value};
\node[box] (env)  at (3.75, 0.3) {environment};
\node[box] (diag) at (6.1, 0.3) {diagnostic};
...
All execution, debugging, transpilation, and static analysis logic is
identical between the two builds.

The WASM bridge exposes the toolchain to JavaScript via
\texttt{EMSCRIPTEN\_KEEPALIVE}-annotated functions (selected):
...
char* pseudo_transpile(const char* src,
                       const char* lang);
char* pseudo_check(const char* source);
\end{lstlisting}
...
box-drawing bar │ $\to$ four spaces, $\blacksquare$ $\to$ \texttt{sf}).
...
\begin{lstlisting}[language=C, caption={C code generated by the transpiler for the program in Listing~\ref{lst:example}.}, label=lst:transpile, float=tp]
#include <stdio.h>
#include <math.h>
...
automated support for it.  Listing~\ref{lst:equiv} shows a \texttt{pentru}
loop converted to an equivalent \texttt{cat timp} loop.

\begin{lstlisting}[language=pseudo, caption={Loop equivalence: a \texttt{pentru} (for) loop (top) converted to a \texttt{cat timp} (while) loop (bottom).}, label=lst:equiv]
# original
...
The key structural distinction is between \emph{pre-test} loops
(\texttt{for}, \texttt{while}) that may execute zero times and
\emph{post-test} loops (\texttt{do-while}, \texttt{repeat}) that always
execute at least once.  Two rewrite primitives cover all nine supported
directed conversions: a \emph{guard} (wrapping the target loop in
\textbf{if}~$C$~\textbf{then}) preserves zero-iteration semantics when
converting from pre-test to post-test; \emph{body duplication} (prepending
one copy of the body before the loop) handles the reverse direction.
Converting \emph{to} a \texttt{for} loop is not supported, as it would
require synthesising a counter variable from an arbitrary loop body.
...
Each diagnostic carries a severity (\texttt{error}, \texttt{warning}, or
\texttt{info}), a code, a 1-indexed source line and column, and a
Romanian-language message.  The analyzer produces ten warning codes
(\texttt{W001}--\texttt{W010}), one informational code (\texttt{W011}),
and one error code (\texttt{E001}).  If syntax errors are present, semantic
checks are suppressed to avoid cascading false positives.

\emph{Per-node checks} (DFS):
\texttt{W001} flags division or modulo by a literal zero;
\texttt{W002} flags self-assignment (\texttt{x <- x});
\texttt{W004} flags empty block bodies.

\emph{Constant-expression evaluation with propagation:}
The analyzer embeds a constant folder covering arithmetic, comparisons,
and logical connectives, including complementary identities
(\texttt{A SI NOT A} $\equiv$ false, \texttt{A SAU NOT A} $\equiv$ true).
A sequential propagation pass additionally tracks known variable values
across statements, enabling pattern detection such as \texttt{x <- 5;
daca x = 5 atunci\ldots}.
\texttt{W003} reports statically-constant conditions (always-true or
always-false); \texttt{W008} reports the unreachable code that results ---
dead branches in conditionals and code following infinite loops.

\emph{Multi-statement pattern checks:}
\texttt{W005} detects the manual swap triplet
($t \leftarrow a$; $a \leftarrow b$; $b \leftarrow t$)
and suggests the built-in \texttt{a $\leftrightarrow$ b} operator;
\texttt{W009} detects a value read from input and immediately overwritten
before any use.

\emph{Usage and control-flow checks:}
\texttt{W006} flags modification of a \texttt{pentru} loop counter inside
its own body; \texttt{W007} performs a two-pass whole-program analysis to
identify variables assigned but never subsequently read;
\texttt{W010} detects shadowed loop variables in nested \texttt{pentru} loops.

\emph{Informational hints:}
\texttt{W011} suggests simplifications for redundant boolean expressions
(\texttt{x SI x} $\to$ \texttt{x}, \texttt{NOT NOT x} $\to$ \texttt{x}).

The \texttt{check} subcommand prints each diagnostic as a single line
(\texttt{avertisment[CODE] la linia L, coloana C: mesaj}).  The
\texttt{pseudo\_check} WASM export serialises all diagnostics as a JSON
array for the web editor's inline gutter markers.
...
\caption{The Pseudo++ web editor with Visual Mode, step-through debugger, and
  ...
  \texttt{x} is assigned but never read (\texttt{W007}).
...
avertisment[W007] la linia 2, coloana 1: Variabila 'x' este definita dar niciodata folosita
...
The \texttt{check} module is covered by 20 unit tests, two per diagnostic
code: one positive case that triggers the warning and one negative case that
confirms its absence.  All 20 tests pass.
...
backend logic; deepening the existing static analysis with full data-flow
diagnostics and richer constant propagation; adding snapshot-based step-back
debugging; and integrating the WASM module into existing Romanian educational
platforms such as pbinfo.ro as a native pseudocode execution target.

Pseudo++ is available as open-source software together with the full
integration test suite at \url{https://github.com/BujorelActimel/pseudo}.
